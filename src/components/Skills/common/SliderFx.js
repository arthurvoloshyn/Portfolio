...
class SliderFx {
  ...
  _morphSVGs = callback => {
    ...
    setTimeout(
      () => currItem.path.stop().animate({ path: pathRectangle }, speed * 3, window.mina.elastic),
      speed * 0.5,
    );
  };
  ...
  _initEvents = () => {
    if (this.itemsCount > 1) {
      ...
      this.support
        ? this.itemsList.addEventListener(window.transEndEventName, transitionEndFn)
        : transitionEndFn();
